/* Sorterer regionerne efter antal store byer, flest store byer først. Dernæst alfabetisk efter regionsnavn. */
...
int region_compare(const void *ip1, const void *ip2){
  region *r1 = (region *) ip1,
         *r2 = (region *) ip2;

  if (int_compare(r1->number_of_large_towns, r2->number_of_large_towns) != 0)
    return - int_compare(r1->number_of_large_towns, r2->number_of_large_towns);
  else  
...
int main(void) {
  ...
  printf("Foer sortering:\n");
  for (i = 0; i < 5; i++)
    print_region(regions, i);

  printf("\n");

  qsort(regions, NUMBER_OF_REGIONS, sizeof(region), region_compare); 
